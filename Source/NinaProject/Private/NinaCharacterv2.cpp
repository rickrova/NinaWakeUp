...
ANinaCharacterv2::ANinaCharacterv2() {
	MinVelocityToJump = 300;
	JumpVelocityFactor = 0.5f;
	FallVelocityFactor = 0.25f;
	CameraFollowThereshold = 3.0f;

	//TimerDel = FTimerDelegate::CreateUObject(this, &ANinaCharacterv2::ResetJumpValues);
	//TimerDel.BindUFunction(this, FName("ResetJumpValues"));
}

void ANinaCharacterv2::BeginPlay() {
	Super::BeginPlay();
}

void ANinaCharacterv2::Tick(float DeltaTime) {
...
void ANinaCharacterv2::OnMovementModeChanged(EMovementMode PrevMovementMode,
...
/*void ANinaCharacterv2::Climb(FHitResult HitResult) {
...
void ANinaCharacterv2::Push(FHitResult HitResult) {
	bBased = true;
	BaseActor = (ABaseMovableActor*)HitResult.GetActor();
	//BaseActor->SetPushingActor(this);
}

void ANinaCharacterv2::StopPush() {
	bBased = false;
	/*bReverse = false;
	BaseActor->RemovePushingActor();*/
}

void ANinaCharacterv2::JumpNow() {
...
void ANinaCharacterv2::CancelMove() {
	GetController()->SetIgnoreMoveInput(true);
}

void ANinaCharacterv2::ResetMove() {
	bPendingJump = false;
	bJumping = false;
	bClimbing = false;
...
void ANinaCharacterv2::FloatUp(float DeltaTime) {
...
void ANinaCharacterv2::StopJump() {
	OnNinaEndFloat();
	bFloatUp = false;
}

void ANinaCharacterv2::JetpackBoost() {
	APlayerController* PlayerController = (APlayerController*)GetController();
...
/*void ANinaCharacterv2::ResetJumpValues() {
